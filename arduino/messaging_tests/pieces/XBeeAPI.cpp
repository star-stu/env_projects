...
uint16_t XBeeAPI::parse_api_frame(uint16_t n_bytes, uint8_t *buffer, APIFrame *frame) 
{
  // check length
...
uint16_t XBeeAPI::wait_for_api_frame(SoftwareSerial &ser, uint8_t *buffer) 
{
  uint16_t n_bytes = 0;
...
void XBeeAPI::construct_api_frame(uint16_t *n_bytes, uint8_t *frame_bytes, uint16_t n_chars, char *message) {
